...
const target = path.join('D:\\SpotlightWallpaper');
	...
	files.forEach(async file => {
		...
		if (stats.isFile()) {
			...
			if (filename) {
				...
				if (wh != 1 && info.bytes > 102400 && info.width > info.height) {
					await fs.writeFile(filename, data, function (err, data) {
						if (err) {
							console.error(err);
							console.error("update fail:" + filename);
						} else {
							console.log("update success:" + filename);
						}
					})
				}
			}
